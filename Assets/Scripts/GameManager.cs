...
public class GameManager : MonoBehaviour
{
    ...
    private const int initialPlayersCount = 1; // Number of players required to set GPS origin
    ...
    public void ProcessPlayerPayload(PlayerPayload payload)
{
    ...
    // ✅ Initialize GPS origin if not set
    if (!gpsOriginSet)
    {
        baseLat = payload.GNSS.Latitude;
        baseLon = payload.GNSS.Longitude;
        gpsOriginSet = true;

        Debug.Log($"[GameManager] GPS Origin set → baseLat={baseLat}, baseLon={baseLon}");
    }
    ...
    GPSUtils.LogGPSPosition($"Player {normalizedId}", payload.GNSS.Latitude, payload.GNSS.Longitude, baseLat, baseLon);
